...
public record EncryptedFile(byte[] owner, byte[] ciphertext, String fileName, byte[] tag, byte[] nonce)
    ...
    public EncryptedFile(PlainFile plainFile, byte[] key, byte[] owner) throws InvalidCipherTextException,
    ...
    public static EncryptedFile getFromDB(byte[] owner, String fileName) throws SQLException {
    ...
    public static void invalidateTag(byte[] owner, String fileName) throws SQLException,
    ...
    public static void deleteEncryptedFile(byte[] owner, String fileName) throws SQLException {
        if (existsInDB(owner, fileName)) {
            try (Connection conn = DriverManager.getConnection(DB_URL)) {
                try (PreparedStatement stmt = conn.prepareStatement(
    ...
    private static boolean existsInDB(byte[] owner, String fileName) throws SQLException {
        try (Connection conn = DriverManager.getConnection(DB_URL)) {
    ...
    private EncryptedFile(byte[] owner, String fileName, byte[][] parts) {
        this(
                owner,
                // ciphertext
                parts[0],
